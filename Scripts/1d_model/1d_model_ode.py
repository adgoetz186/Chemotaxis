...
run_dict = {"L0":{"repeat":1,"L_init":0},"L001":{"repeat":1,"L_init":0.01},"L01":{"repeat":1,"L_init":0.1},"L05":{"repeat":1,"L_init":0.5},"L1":{"repeat":1,"L_init":1}}
run_batch_list = []
...
for i in range(len(run_batch_list)):
    ...
    result = sim_obj.run(param_values=copy.copy(param_dict),tspan=t_init,method='ode')
    ...
    for j in range(t_steps):
        ...
        dict_EGF = dict(zip(EGF_names, EGF_values))
        param_dict.update(dict_EGF)
        result = sim_obj.run(param_values=copy.copy(param_dict), tspan=[t,t+t_step], method='ode', initials=result.dataframe.to_numpy()[-1, :])
        new_run_dataframe = result.dataframe
    ...
    #input()
    if not os.path.exists(f"data/Simulations_1d_diff_det/{run_batch_list[i]}"):
        os.mkdir(f"data/Simulations_1d_diff_det/{run_batch_list[i]}")
    run_dataframe.to_csv(f"data/Simulations_1d_diff_det/{run_batch_list[i]}/{run_batch_list[i]}_{len(os.listdir(f'data/Simulations_1d_diff_det/{run_batch_list[i]}'))}")
    print(result.dataframe)
