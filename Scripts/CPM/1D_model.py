...
def single_copy_sim(cell_dtf, A0=10, lambda_val=0.01, alpha=2, buffer=5):
	# PLACEHOLDER
...
dtf_dict = {'kprod': 0.05876103899761891 / 0.00122, 'kbind': 0.01625578123538074, 'kunbind': 0.39307675090835553,
            'krp': 0.4777926053914779, 'krdp': 0.04702130222751365, 'kdeg': 0.000329582245342056,
            'ksdeg': 0.0010486403956587256, 'R_0': 0, 'B_0': 0, 'P_0': 0, 'EGF': 1}
AZ = time.time()
sims = 0
...
for traj_number in range(10):
	Cell_df = pd.DataFrame(
		{'x_val': x_vals, 'y_val': y_vals, "R": [dtf_dict['kprod'] / len(x_vals) for i in range(len(x_vals))],
		 "B": [0.0 for i in range(len(x_vals))], "P": [0.0 for i in range(len(x_vals))]})
	if not os.path.exists(f"data/trajectories/traj_{traj_number}"):
		os.mkdir(f"data/trajectories/traj_{traj_number}")
	...
	for repeat in range(400):
		...
		for i in range(10):
			...
			Cell_df.to_csv(path_or_buf=f"data/trajectories/traj_{traj_number}/traj_{repeat}_{i}")
