...
def L_diff_new(x, grad, l_loc, length):
	ph_val = np.linspace(grad*l_loc, grad*(l_loc+length), np.size(x))
	# print(ph_val)
	return ph_val

def solve(dt, dx, D, L, T, T_step, k,grad,l_loc,length):
	...
	L_values[0] = L_diff_new(X,grad,l_loc,length)
...
results = solve(dt,dx,D,L,T_count,T_stim,kvec,grad_drop,l_loc,length)
